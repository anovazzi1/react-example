...
export default function Content({tableSpecs,contentData}){
    ...
    return (
        <td colSpan={6} className="text-sm pr-6 whitespace-no-wrap text-gray-800 dark:text-gray-100 tracking-normal leading-4">
            <TokenAnnotator contentEditable="true" spellCheck="false" className={`h-${contentData.tokens} text-${tableSpecs.contentTextSize}`} tokens={text.split(' ')} value={contentData.explanation}></TokenAnnotator>
        </td>
    )
}